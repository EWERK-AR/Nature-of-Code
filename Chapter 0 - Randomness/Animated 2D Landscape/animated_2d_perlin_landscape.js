...
function draw() {
    ...
    for (let x = 0; x < width; x++) {
        ...
        for (let y = 0; y < height; y++) {
            ...
            // Map the noise value (0-1) to elevation (0-255)
            let elevation = map(n, 0, 1, 0, 255);

            // Base brightness depending on elevation
            let colorBase = map(elevation, 0, 255, 0, colourSlider.value());

            // Create color variables
            let r, g, b;

            // If elevation is low -> darker (valleys)
            if (elevation < 128) {
                r = colorBase * 0.6; // * 1 = same brightness -> * 0.6 (<1) = darker
                g = colorBase * 0.8;
                b = colorBase;
            }

            // Otherwise elevation is high -> brighter (mountains)
            else {
                r = colorBase;
                g = colorBase;
                b = colorBase;
